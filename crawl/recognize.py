...
class RecognizeDigitCheckCode():
    ...
    def predictImgBase64Str(self, base64_str):
...
methods2 = ['cv.TM_CCOEFF', 'cv.TM_CCOEFF_NORMED', 'cv.TM_CCORR', 'cv.TM_CCORR_NORMED', 'cv.TM_SQDIFF', 'cv.TM_SQDIFF_NORMED']
...
def writeChineseTemplate(texts):
    
    i = 0
    for text in texts:
        img = formChineseTemplate(text)
        cv.imwrite("chinese%d.png"%i, img)
        i += 1
...
def recognizeChineseCharFromFile(pathtofile, text):
    img               = cv.imread(pathtofile)
